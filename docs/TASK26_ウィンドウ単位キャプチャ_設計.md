...
### P0 の実測結果（2026-09-07 / 合格）

`native/window_capture/src/main.cpp` を実装し、**このPCで実際に確認した。**

#### 1. 重なりが映らないこと（★この機能の全目的）

対象（Discord = Electron）の中央50%を**純マゼンタの最前面ウィンドウ**で覆い、
その状態で新方式と現行方式を**同時に**撮って、マゼンタ画素の割合を数えた。

| 方式 | 寸法 | mean | **マゼンタ** |
|---|---|---|---|
| **WGC（新）** | 1280x1392 | 36.3 | **0.00%** |
| gdigrab 切り出し（現行・対照） | 1286x1398 | 69.9 | **25.16%** |

- **WGC 側にマゼンタは1画素も出なかった。** 重なりは映らない
- 対照側には 25.16% 出ている → **覆いは確かに置かれていた**（テストが成立している）
- WGC 側の `mean=36.3` はタスク23の実測値（30.6 / 37.2 / 133.7）と同程度。
  **真っ黒（0.0）でも真っ白（255.0）でもない** = GPU合成ウィンドウが正しく映っている
- 目視ではなく画素数で判定した。タスク23で「rc=0 かつ h264 生成済みなのに真っ黒」を
  すり抜けた前科があるため

#### 2. 静止画面でもフレームが途切れないこと

4秒 × 10fps に対し **40フレームちょうど**（端数0バイト）。
`dup=36` = 36回は「新しい絵が無いので直前を再送」。**壁時計送出は効いている。**

#### 3. 本番経路（名前付きパイプ → ffmpeg → h264）

対象 2560x1440、30fps、10秒:

| 項目 | 実測 |
|---|---|
| ハンドシェイク（起動→ready行） | **0.23 秒** |
| h264 のフレーム数 | **300 / 300**（実効 30.0 fps） |
| 生BGRAの帯域 | **442 MB/s** |
| 補助exeのCPU | 5.05秒 / 10秒 = **コア換算 50%** |
| resize | 0 |

- **442MB/s を名前付きパイプで捌けている。** §5.3 の「まず BGRA で測る」は
  これで決着。**NV12 化は不要**（推測で先回りしなくて正解だった）
- 現行方式は同等サイズ（2568x1401）で実効23.7fpsまで落ちていた。
  **WGC は 30.0fps を維持する** → 重なりだけでなく速度も改善している

#### 4. 出力寸法は Python 側で決められないことの確認

`GetWindowRect` は **1294x1399**、WGC の item size は **1280x1392**。
DWM の影・枠のぶんズレる。**§4.3 のハンドシェイクは必須**だと実地で確認できた。

#### 未測定（P1実機テストへ持ち越し）

- **覆われたときにアプリ自身が描画を止める件（§5.5）。** WGC で改善するかは未測定。
  UIの注意書きは**「重なりは映らない」だけを断定**し、fpsの話は「止まることがある」に
  留めてある。測る前に断定しないこと
- 黄色い枠（`--no-border`）の目視確認
- リサイズ中の挙動（`resize` カウンタは 0 のまま = 発火していない）
...
## 8. P0 追加実測（2026-09-07）— 現行方式との比較

同一ウィンドウ（VRCX 1637x974）に対し、10秒 @30fps を同条件で撮り比べた。
CPU は「取り込み + 送出エンコード」の合計。

| 方式 | 寸法 | frames(期待300) | CPU | コア換算 |
|---|---|---|---|---|
| **WGC（新）** | 1622x966 | **300** | 4.78s | **48%** |
| gdigrab 切り出し（現行） | 1636x974 | **269** | 5.72s | 57% |
| WGC（3/4を覆った状態） | 1622x966 | **300** | 3.92s | 39% |

- **WGC の方が速くて軽い。** 現行方式は 269/300 = 実効26.9fps までフレームを落として
  いるのに CPU は多く食っている。§6 の合格条件「CPU が現行方式を上回らない」は満たした
- **NV12 化は不要**と確定（§5.3）。BGRA のまま 442MB/s を捌けており、
  推測で先回りしなくて正解だった

### 覆われたときの描画抑制（§5.5）は**この測定では決着しなかった**
...
対象に選んだ VRCX がそもそもほぼ静止しており（覆いなしでも 1.1fps しか更新が無い）、
**覆いの有無による差を測れていない**。覆った方が数値が良いのは再描画が誘発されただけで、
改善の証拠にはならない。

→ **動きのあるウィンドウで実機テストすること。**
[TASK26_実機テスト手順.md](TASK26_実機テスト手順.md) の 2-4 に手順を置いた。
測るまで UI の注意書きに「覆っても滑らかに動く」とは書かない。

## 9. P1 の実測結果（2026-09-07 / 合格）

`streamer_core.py` の本番関数だけを使い、重なりを置いた状態で通した:

`start_window_capture_helper` → `build_screen_capture_input` → `build_screen_video_filter`
→ ffmpeg → h264

| 項目 | 実測 |
|---|---|
| ハンドシェイク | `ready size=1280x1392`（`GetWindowRect` は 1294x1399） |
| `needs_hwdownload` | `False`（正しい。hwdownload は ddagrab 専用） |
| 出力 | **1920x1080**、180/180 フレーム（6秒×30fps） |
| 絵 | `mean=18.1`（黒でも白でもない） |
| **重なり** | **0.00%** — 本番経路でも映らない |

### P1 で見つけて直した回帰

`ui/index.html` を書き換えたのに配布用の複製 `plugin/ui/index.html` を同期しておらず、
UI同期テスト4件が落ちていた（`test_host_only_mode` / `test_clock_features` /
`test_destination` / `test_hw_encoding`）。**元コミットでは通るので、この変更が入れた回帰。**
複製を同期して解消。

`test_transition` と `test_yt_dlp` は元コミット `f68623c` でも同様に落ちるため、
このタスクとは無関係（サーバ/ネットワークを要する環境依存）。

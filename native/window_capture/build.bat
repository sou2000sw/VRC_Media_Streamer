@echo off
rem ==========================================================================
rem  window_capture - build script (MSVC x64)
...
cl /nologo /std:c++17 /utf-8 /EHsc /O2 /W4 /DUNICODE /D_UNICODE ^
   /Fe:"%OUTDIR%\window_capture.exe" ^
   /Fo:"%OUTDIR%\\" ^
   "%SRC%" ^
   /link windowsapp.lib d3d11.lib dxgi.lib dxguid.lib user32.lib winmm.lib
...
echo [build] OK -^> %OUTDIR%\window_capture.exe
endlocal

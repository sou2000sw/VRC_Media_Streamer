...
| **17** | 🎵 ラジオ | **ラジオモード時の曲間フェード** (Smooth Fade) | `feature/obs-style-ui` | 🟡 **実装完了・実機未確認** |
| **18** | ⚡ 性能 | **FFmpeg ハードウェアエンコード（NVENC / QSV / AMF）対応** (HW Encoding) | `feature/obs-style-ui` | 🟢 **実装完了 ✅** |
...
| **22** | 🎙️ 音声配信 | **PC出力音声（ループバック）＆マイク入力音声の取り込み・配信** (PC Audio & Mic Capture) | `feature/obs-style-ui` | 🟡 **実装完了・VRC実機未確認** |
| **23** | 🖥️ 画面配信 | **PCデスクトップ画面・ウィンドウのリアルタイムキャプチャ配信** (Desktop Screen Share) | `feature/obs-style-ui` | 🟡 **実装完了・VRC実機未確認** |
| **24** | 🎤 参加型 | **Webリモコンからの参加型カラオケ・楽器セッション機能** (Remote Karaoke & Session) | — | ⚪ **No.27 として実装（本項は設計草案として保存）** |
| **25** | 🎚️ 音声配信 | **アプリ単位の音声取り込み（WASAPIプロセスループバック）** (Per-Application Audio Capture) | `feature/obs-style-ui` | 🟡 **実装完了・ホスト側実機確認済み / VRChat内は未確認** |
| **26** | 🪟 画面配信 | **ウィンドウ取り込みの WGC 移行（手前に重なった窓を映さない）** (WGC Window Capture) | `feature/obs-style-ui` | 🟡 **実装完了・VRC実機未確認** |
| **27** | 🎤 参加型 | **参加型カラオケ・セッション（Webリモコンがマイクになる／生演奏の双方向監聴）** (Remote Karaoke Session) | `feature/obs-style-ui` | 🟡 **実装完了・VRC実機未確認** |
| **28** | 🎛 UI/音声 | **操作画面のOBS風3ペイン化（シーン / ソース / 音声ミキサー）＋素材音量フェーダー** (OBS-style Console) | `feature/obs-style-ui` | 🟡 **実装完了・実機未確認** |

> [!important] ブランチ現況（2026-09-07 時点）
> - `main` … 公開版。`feature/obs-style-ui` から **164コミット遅れ**
> - `develop` … **No.21（ホスト専用モード）まで**。2026-09-04 で止まっている
> - `feature/obs-style-ui` … 作業中の先端。`develop` より **70コミット先行**し、
>   **No.17 / 18 / 22 / 23 / 25 / 26 / 27 / 28 はこのブランチにしか存在しない**
> - `version.py` は **v2.10.6**。ただし `CHANGELOG.md` には **2.9.9〜2.10.6 の見出しが無く、
>   すべて `[Unreleased]` に積まれている**（リリース番号を切る作業が未了）
...
## 25. 🎚️ アプリ単位の音声取り込み (Per-Application Audio Capture) 【実装完了・VRC実機未確認 🟡】

### 概要
Discord の「アプリケーション音声キャプチャ」と同じく、**選んだウィンドウのプロセスが鳴らしている音だけ**を配信に乗せる。PC出力を丸ごと取る従来のループバックと違い、通知音や別アプリの音が混ざらない。

### 実装（`feature/obs-style-ui`）
- WASAPI の**プロセスループバック**。ffmpeg に該当バインディングが無いため、補助exe `native/app_audio_capture` を新設し、生PCMを取り込み側FFmpegの `stdin` へOSパイプで直結
- マイク / PC出力音声と同時に使える（最大3系統を `amix` で合成）
- **補助exeが無い・対象ウィンドウが見つからない場合でも配信は止まらない**（従来の2系統で続行）

### 実測でひっくり返った前提
- 「無音のプロセスはパケットが来ないので時間軸がずれる」と想定していたが、**完全無音30秒で誤差0**。ずれない
- 対象アプリの 440Hz を 440.0Hz で復元し、同時に鳴っている無関係プロセスへ漏れない（分離が成立）

### 既知の制限
- ストアアプリ（UWP）はウィンドウのPIDが `ApplicationFrameHost.exe` を指すため取り込めないことがある

---

## 26. 🪟 ウィンドウ取り込みの WGC 移行 (WGC Window Capture) 【実装完了・VRC実機未確認 🟡】

### 概要
タスク23のウィンドウ取り込みは「合成済みデスクトップの切り出し」だったため、**手前に別のウィンドウが重なるとそれごと映っていた**。`Windows.Graphics.Capture`（WGC）へ切り替え、選んだウィンドウの中身だけを取り込む。

### 実装（`feature/obs-style-ui`）
- ffmpeg に WGC 入力が無いため、タスク25と同じ方式で補助exe `native/window_capture/` を新設
- 重なりの検証は**目視ではなく画素数**で判定（重ねた色の混入率を数える）

### 残る制約
- 共有元ウィンドウを**最小化**すると描画が止まる（WGCの仕様。裏に隠れるだけなら継続する）

---

## 27. 🎤 参加型カラオケ・セッション (Remote Karaoke & Live Session) 【実装完了・VRC実機未確認 🟡】

No.24 の設計草案を実装したもの。**通信方式は草案の「アプローチA（WebSocket / リニアPCM）」を採用**した。

### 実装（`feature/obs-style-ui`）
- **WSS**（16bit / 48kHz / モノラル、40msごと）。Cloudflare Tunnel をそのまま通るので UDP開放・STUN/TURN が要らない
- **既定は全部閉じている**（受付オフ＋承認制）。知らないうちに他人の声が配信へ乗ることは起こらない
- ホスト卓に参加者ごとの音量 / PAN / ミュート / 退出、全体のマスター音量 / リバーブ / 遅延補正、🔴セッション録音。**配信中に動かしても配信は途切れない**
- 生演奏の双方向対応（ホストのマイクを参加者へ配る／ホストが参加者を監聴する）

### 実装して分かった要点
- **遅延補正の符号は「誰に合わせるか」で逆になる**。ホスト基準なら歌声を早める（−）、参加者基準ならホストを遅らせる（＋）。卓で基準を選ばせる形にした
- **どちらの向きでも「自分の音」は返さない**。返ると往復ぶん遅れた山びこになり演奏できない
- 監聴は**ホストPCからの接続のみ**受理する。開けると参加者どうしが互いの声を盗み聴きできる
- 足し合わせる場所に**余裕（ヘッドルーム）が要る**。3か所とも天井で頭打ちにするだけだったため、市販音源のピーク30000と歌声を混ぜると約20%のサンプルが天井に貼り付いて歪んでいた

### 残課題
- VRChat 実機での通しの確認（遅延補正の実用値、複数人同時）

---

## 28. 🎛 操作画面のOBS風3ペイン化＋素材音量フェーダー (OBS-style Console) 【実装完了・実機未確認 🟡】

### 概要
マイクやPC出力の音量が**設定タブの一番奥**にあり本番中に触れなかった。配信ソフトと同じ形（左=シーン / 中央=ソース / 右=音声ミキサー）へ組み替え、プレイヤータブの先頭に常設した。

### 実装（`feature/obs-style-ui`）
- ミキサーは**実質「移動」**。`mic` / `loopback` / アプリ音声は元から `amix` で合成されており、データモデルが既にOBSのミキサーと一致していた。id を変えていないため既存のJS（`loadAudioDevices` 等）は無改修で効く
- シーンは既存の `playback_mode`（`video` / `radio` / `slideshow` / `live` / `screen`）をそのまま縦リストにしたもの
- **素材音量フェーダー（`media_volume`, 0.0〜2.0）を新設**。動画・ラジオの音を絞れず、マイクを乗せると負けていた
- **効かないチャンネルはシーンに応じて薄く落とす**（取り込み系は `live`/`screen` のみ、素材音量はその逆で `video`/`radio`/`slideshow` のみ有効）

### 設計上の固定事項
- **既定値では FFmpeg コマンドを1文字も変えない**（等倍なら `volume=` を差し込まない）。「音量を触っていないのに音が変わった」という切り分け不能な回帰を作らないため。3経路それぞれテストで固定した
- **同じ値で呼ばれても張り直さない**。UIは操作のたび現在値を送るため、素直に `request_stream_reload()` すると値が変わっていないのに配信が切れる
- OBS と違い**シーン切替は無料ではない**（FFmpegが張り直り数秒切れる）ので、デッキ先頭に常時警告帯を置いた
